...
module.exports.cssFile = function(req, rep) {
	...
	var absPath = Path.join(__dirname, '../assets/static/css/' + cssPath);
	var isExit = fs.existsSync(absPath);
	if (isExit) {
...
module.exports.imgFile = function(req, rep) {
	...
	var absPath = Path.join(__dirname, '../assets/static/images/' + imgPath);
	var isExit = fs.existsSync(absPath);
...
module.exports.jsFile = function(req, rep) {
	...
	var absPath = Path.join(__dirname, '../assets/static/js/' + jsPath);
	var desPath = Path.join(__dirname, '../assets/static/js/' + jsPath.replace(/\.js/g, '.dev.js'));
	var isExit = fs.existsSync(absPath);
	if (isExit) {
...
module.exports.fontFile = function(req, rep){
	var filePath = req.params.fontsPath;
	var absPath = Path.join(__dirname, '../assets/static/fonts/' + filePath);
	var isExit = fs.existsSync(absPath);
	return isExit ? rep.file(absPath) : rep(Boom.notFound('missing'));
};
...
module.exports.plugpackFile = function(req, rep) {
    var filePath = req.params.filePath;
    var absPath = Path.join(__dirname, '../plug/' + filePath);
    var isExit = fs.existsSync(absPath);
    return isExit ? rep.file(absPath) : rep(Boom.notFound('missing'));
};